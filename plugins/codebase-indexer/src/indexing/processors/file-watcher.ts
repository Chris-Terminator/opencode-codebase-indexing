...
export class FileWatcher implements IFileWatcher {
  ...
  async initialize(): Promise<void> {
    ...
    this.watcher = chokidarWatch(this.workspacePath, {
      ignored: (filePath: string) => {
        ...
        if (this.isIgnoreRulesFile(relativeFilePath)) return false
  ...
  private isIgnoreRulesFile(relativePath: string): boolean {
    const normalized = relativePath.replaceAll("\\", "/")
    return normalized === ".gitignore" || normalized === ".kilocodeignore"
  }

  private async reloadIgnoreRules(): Promise<void> {
    ...
    if (newlyIgnored.length === 0) return

    await this.vectorStore.deletePointsByMultipleFilePaths(newlyIgnored)
    for (const filePath of newlyIgnored) this.cacheManager.deleteHash(filePath)
  }
