...
import { OLLAMA_EMBEDDER_REQUEST_TIMEOUT_MS, REMOTE_EMBEDDER_VALIDATION_TIMEOUT_MS, } from "./constants";
import { Log } from "../util/log";
const log = Log.create({ service: "indexing-factory" });
function timeout(provider) {
    if (provider === "ollama")
        return OLLAMA_EMBEDDER_REQUEST_TIMEOUT_MS;
...
export class CodeIndexServiceFactory {
    ...
    createEmbedder() {
        ...
        if (provider === "kilo")
            throw new Error("Kilo-hosted embeddings are not supported by this Codex plugin.");
        if (provider === "openai") {
        ...
        if (provider === "gemini") {
            if (!config.geminiOptions?.apiKey)
                throw new Error("Gemini API key is required for embedding.");
            return new GeminiEmbedder(config.geminiOptions.apiKey, config.modelId);
        }
        if (provider === "mistral") {
            if (!config.mistralOptions?.apiKey)
                throw new Error("Mistral API key is required for embedding.");
        ...
        if (provider === "bedrock") {
            if (!config.bedrockOptions?.region)
                throw new Error("Bedrock region is required for embedding.");
        ...
        if (provider === "voyage") {
            if (!config.voyageOptions?.apiKey)
                throw new Error("Voyage API key is required for embedding.");
    ...
    async validateEmbedder(embedder) {
        ...
        const fail = new Promise((resolve) => {
            timer = setTimeout(() => resolve({
                valid: false,
                error: embedder.embedderInfo.name === "ollama"
    ...
    createVectorStore() {
        ...
        if (!profile || profile.dimension <= 0) {
            throw new Error(`Cannot determine vector dimension for model "${config.modelId ?? getDefaultModelId(config.embedderProvider)}" with provider "${config.embedderProvider}". ` +
    ...
    createDirectoryScanner(embedder, vectorStore, parser, ignoreInstance) {
        ...
        return new DirectoryScanner(embedder, vectorStore, parser, this.cacheManager, ignoreInstance, config.embeddingBatchSize, config.scannerMaxBatchRetries, this.onTelemetry, meta);
    }
    createFileWatcher(embedder, vectorStore, cacheManager, ignoreInstance) {
        const config = this.configManager.getConfig();
        const meta = this.getTelemetryMeta();
        return new FileWatcher(this.workspacePath, cacheManager, embedder, vectorStore, ignoreInstance, config.embeddingBatchSize, config.scannerMaxBatchRetries, this.onTelemetry, meta);
    }
    createServices(cacheManager, ignoreInstance) {
