...
import { QDRANT_CODE_BLOCK_NAMESPACE, MAX_FILE_SIZE_BYTES, BATCH_SEGMENT_THRESHOLD, MAX_BATCH_RETRIES, INITIAL_RETRY_DELAY_MS, PARSING_CONCURRENCY, BATCH_PROCESSING_CONCURRENCY, MAX_PENDING_BATCHES, } from "../constants";
...
export class DirectoryScanner {
    ...
    maxBatchRetries;
    constructor(embedder, vectorStore, codeParser, cacheManager, ignoreInstance, batchSegmentThreshold, maxBatchRetries, onTelemetry, telemetryMeta) {
    ...
    emitFileCount(mode, discovered, candidate) {
    ...
    emitRetry(mode, attempt, batchSize, err) {
        if (!this.onTelemetry || !this.telemetryMeta) {
            return;
        }
    ...
    emitError(mode, location, err, retryCount) {
        if (!this.onTelemetry || !this.telemetryMeta) {
            return;
        }
        const msg = err instanceof Error ? err.message : String(err);
     ...
     */
    async scanDirectory(directory, onError, onFilesIndexed, onFileParsed, mode = "full") {
        ...
        const queueBatch = async (batchBlocks, batchTexts, batchFileInfos) => {
            while (!this._cancelled) {
                ...
                try {
                    if (pendingBatchCount < MAX_PENDING_BATCHES) {
                        pendingBatchCount++;
                        const batchPromise = batchLimiter(() => this.processBatch(batchBlocks, batchTexts, batchFileInfos, scanWorkspace, mode, onError, onFilesIndexed, () => {
                            failed = true;
                        }));
    ...
    async processBatch(batchBlocks, batchTexts, batchFileInfos, scanWorkspace, mode, onError, onFilesIndexed, onBatchFailed) {
            ...
            try {
                ...
                if (uniqueFilePaths.length > 0) {
                    try {
                        ...
                        log.error(`Failed to delete points for ${uniqueFilePaths.length} files before upsert in workspace ${scanWorkspace}`, {
                            error: sanitizeErrorMessage(errorMessage),
