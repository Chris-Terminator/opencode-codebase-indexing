...
export class CodeIndexOrchestrator {
    ...
    constructor(configManager, stateManager, workspacePath, cacheManager, vectorStore, scanner, fileWatcher, onTelemetry) {
    ...
    emitError(location, err, source, trigger, mode) {
    ...
    async _startWatcher() {
        ...
        try {
            ...
            this._fileWatcherSubscriptions = [
                ...
                this.fileWatcher.onDidFinishBatchProcessing.on((summary) => {
    ...
    async startIndexing(trigger = "background") {
    ...
    async _runScan(mode, trigger) {
        ...
        const handleFilesIndexed = (indexedCount) => {
        ...
        }, handleFilesIndexed, handleFileParsed, mode);
        ...
        if (mode === "full") {
            ...
            if (batchErrors.length > 0) {
                ...
                if (failureRate > 0.1) {
                    ...
                    throw new Error(`Indexing partially failed: Only ${cumulativeFilesIndexed} of ${cumulativeFilesFound} files were indexed. ${msg}`);
                }
            }
        }
    ...
    stopWatcher() {
        ...
        this.scanner.cancel();
        for (const sub of this._fileWatcherSubscriptions)
            sub.dispose();
        this._fileWatcherSubscriptions = [];
        if (this.stateManager.state !== "Error") {
