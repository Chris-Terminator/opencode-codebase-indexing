...
export class CodeIndexManager {
    ...
    _retryTask;
    _retryAttempt = 0;
    _retryMaxAttempts = MAX_MANAGER_RECOVERY_ATTEMPTS;
    _retryInitialDelayMs = INITIAL_MANAGER_RECOVERY_DELAY_MS;
    _disposed = false;
    constructor(workspacePath, cacheDirectory) {
    ...
    emitError(location, err, trigger) {
        const meta = this.getTelemetryMeta();
        if (!meta) {
            return;
        }
    ...
    handleTelemetry(event) {
        ...
        if (event.type !== "error")
            return;
        if (event.location !== "orchestrator:startIndexing")
            return;
        if (!this.isFeatureEnabled || !this.isFeatureConfigured)
            return;
        if (this._retryTask || this._isRecoveringFromError)
            return;
    ...
    async runRecovery(trigger, attempt) {
        if (this._disposed)
            return;
        ...
        try {
            ...
            this._stateManager.setSystemState("Error", `Failed during recovery: ${err instanceof Error ? err.message : String(err)}`);
        }
    ...
    async initialize(input) {
        ...
        if (!this.isFeatureConfigured) {
            ...
            this._stateManager.setSystemState("Standby", "Code indexing is not configured. Save your settings to start indexing.");
            return { requiresRestart };
        }
        ...
        if (needsServiceRecreation) {
            try {
                ...
                this._stateManager.setSystemState("Error", `Failed to initialize: ${err instanceof Error ? err.message : String(err)}`);
                throw err;
            }
        }
        const shouldStartOrRestart = requiresRestart || (needsServiceRecreation && (!this._orchestrator || this._orchestrator.state !== "Indexing"));
    ...
    async recoverFromError(trigger = "background") {
        if (this._disposed)
            return;
        if (this._retryTask) {
    ...
    async _recreateServices() {
        ...
        this._orchestrator = undefined;
        this._searchService = undefined;
        this._serviceFactory = new CodeIndexServiceFactory(this._configManager, this.workspacePath, this._cacheManager, this.cacheDirectory, (event) => this.handleTelemetry(event));
        const ignoreInstance = await loadIgnore(this.workspacePath);
        const config = this._configManager.getConfig();
        const { embedder, vectorStore, scanner, fileWatcher } = this._serviceFactory.createServices(this._cacheManager, ignoreInstance);
        ...
        this._orchestrator = new CodeIndexOrchestrator(this._configManager, this._stateManager, this.workspacePath, this._cacheManager, vectorStore, scanner, fileWatcher, (event) => this.handleTelemetry(event));
